...
def register_new_usr(name_sign_up: str, email_sign_up: str, username_sign_up: str, password_sign_up: str) -> None:
    ...
    # with open("data/registered_users.txt", "a") as f:
    #     f.write(f"Username: {username_sign_up} | Password: {password_sign_up} | Email: {email_sign_up}\n")

    # Send credentials to user's email (using your existing send_passwd_in_email or similar)
    subject = "Your Geetorus Account Credentials"
    message = f"Hello {name_sign_up},\n\nYour Geetorus account has been created.\n\nUsername: {username_sign_up}\nPassword: {password_sign_up}\n\nKeep this information safe."
    send_passwd_in_email(auth_token=None, username=username_sign_up, email=email_sign_up, company_name="Geetorus", random_password=password_sign_up, subject=subject, message=message)
